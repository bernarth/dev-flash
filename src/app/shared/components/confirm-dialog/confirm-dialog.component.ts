...
@Component({
  ...
  styles: [`
    .scrim {
      ...
      background: var(--df-scrim); z-index: var(--df-z-overlay);
      animation: fade-in 150ms ease;
    }
    .sheet {
      position: fixed; left: 0; right: 0; bottom: 0; z-index: var(--df-z-modal);
    ...
    .handle {
      width: 2.5rem; height: 4px; border-radius: var(--df-radius-pill);
      background: var(--df-outline); margin: 0 auto 1.25rem;
    }
    .body { text-align: center; margin-bottom: 1.25rem; }
    .icon-wrap {
    ...
    .title { font-size: 1.125rem; font-weight: 600; letter-spacing: -0.02em; }
    .description {
      font-size: 0.8125rem; color: var(--df-text-muted);
      margin: 0.5rem auto 0; max-width: 18.75rem; line-height: 1.55;
    }
    .actions { display: flex; flex-direction: column; gap: 0.5rem; }
    .confirm-btn {
      height: 3rem; border-radius: var(--df-radius); border: 0;
      font-family: inherit; font-size: 0.875rem; font-weight: 600;
      cursor: pointer; transition: transform 100ms;
    }
    .confirm-btn:active { transform: scale(0.98); }
    .confirm-btn--danger  { background: var(--df-again); color: var(--df-primary-ink); }
    .confirm-btn--warning { background: var(--df-hard);  color: var(--df-primary-ink); }
    .confirm-btn--default { background: var(--df-primary); color: var(--df-primary-ink); }
    .cancel-btn {
      height: 3rem; border-radius: var(--df-radius);
      border: 1px solid var(--df-outline); background: transparent;
      color: var(--df-text); font-family: inherit; font-size: 0.875rem; cursor: pointer;
    }
