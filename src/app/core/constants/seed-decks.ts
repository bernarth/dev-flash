...
export const DEFAULT_DECKS: DefaultDeckSeed[] = [
    ...
    cards: [
      {
        ...
        answer:
          '- `==` compares after type coercion (`"5" == 5` is true).\n- `===` is strict equality: no coercion, types must match (`"5" === 5` is false).\n\nPrefer `===` by default.',
  ...
  },
  {
    ...
    cards: [
      {
        question: 'What is the Common Intermediate Language (CIL)?',
        answer:
          'It is the intermediate language that all .NET-compatible languages like C#, Visual Basic, or F# get compiled to.',
        notes:
          'C#, F# and similar languages are compatible because they all compile to the CIL. Compilation to CIL happens at compile time, not runtime.',
        tags: [],
        nextSession: 0,
      },
      {
        question: 'What is the Common Language Runtime (CLR)?',
        answer: 'It is a runtime environment that manages the execution of .NET applications.',
        notes:
          'The CLR is responsible for many operations, such as:\n\n- JIT compilation\n- Memory management\n- Exception handling\n- Thread management\n- Type safety (CTS)',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- **C#** is a programming language.\n- **.NET** is a framework that supports applications written in C#, as well as in other .NET-compatible languages.',
        notes:
          'C# is a plane and .NET is the airport. .NET provides the execution environment (CLR).',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '1. Value types inherit from `System.ValueType`; reference types inherit from `System.Object`.\n2. Value types are passed by copy; reference types are passed by reference.\n3. On assignment, a variable of a value type is copied; for reference types, only the reference is copied.\n4. All value types are sealed.\n5. Value types are stored on the stack; reference types are stored on the heap (which is why the garbage collector only cleans up reference types).',
        notes:
          '- **Value types:** `int`, `double`, `DateTime`, `bool`.\n- **Reference types:** `object`, `string`, `StringBuilder`, `List<T>`, arrays.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- **Boxing** — wrapping a value type into an instance of `System.Object`.\n- **Unboxing** — the opposite: converting the boxed value back to a value type.',
        notes: 'The penalty is on performance, because boxing is expensive.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '1. **Compilation errors** — also known as syntax errors; reported by the compiler.\n2. **Runtime errors** — thrown during program execution.\n3. **Logical errors** — the program runs without crashing but does not produce a correct result.',
        notes:
          'Both runtime and logical errors can be prevented with unit tests. Exceptions are the mechanism C# uses to handle runtime errors.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'Exceptions are handled with `try`-`catch`-`finally` blocks:\n\n- `try` contains the code that may throw an exception.\n- `catch` defines what to do when an exception of a given type is thrown.\n- `finally` runs no matter whether an exception was thrown or not.',
      ...
      },
      {
        ...
        answer:
          '- `public`\n- `internal`\n- `protected`\n- `protected internal`\n- `private protected`\n- `private`',
        notes:
          '- `private protected` works like `protected`, but only within the same assembly.\n- `protected internal` works like `internal`, but from a different assembly it behaves like `protected`.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- At the **namespace** level (e.g. top-level classes): `internal`.\n- At the **class** level (members): `private`.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'The `sealed` modifier prevents a class from being inherited, or an overridden method from being overridden further.',
        notes:
          'You cannot make an abstract class `sealed` — it makes no sense. Marking a class `sealed` can increase performance, since it tells the CLR to avoid looking for further derived classes.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'The `params` keyword allows passing any number of arguments of the same type to a method.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          "1. Structs are **value types**; classes are **reference types**.\n2. Structs don't support inheritance, although they can implement interfaces.\n3. Structs can't have destructors.\n4. Unlike classes, structs always have a parameterless constructor, even if you don't define one explicitly.",
        notes:
          "Use structs when:\n\n- The type is logically small and small in memory.\n- The type is immutable.\n- The type is commonly embedded in other objects.\n- You want value-type semantics.\n- The struct won't be boxed.",
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'It makes a large source class easier to work with in big projects. Static classes can be partial classes.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'It can be used in three contexts:\n\n1. The `new` **operator** — creates a new instance of a type.\n2. The `new` **modifier** — explicitly hides a member from a base class in the derived class.\n3. The `new` **constraint** — specifies that a type argument in a generic class must have a parameterless constructor.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'It can be used in two contexts:\n\n1. The `static` **modifier** — defines static classes, as well as static members in classes, structs, and records.\n2. The `using static` **directive** — references static members without specifying their type name every time.',
        notes:
      ...
      },
      {
        ...
        answer:
          'A static class is a class that cannot be instantiated and can only contain static members. It can work as a container for methods that just operate on input parameters and do not get or set any internal instance state.',
        notes:
          'A static class can have a unique constructor — the static constructor — to initialize static fields.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'It is a shorter syntax for an `if`-`else` statement: `condition ? whenTrue : whenFalse`.\n\nIt evaluates a boolean expression and returns one of the two expressions, depending on whether the condition is true or false.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- `??` (null coalescing) — returns the left-hand operand if it is not null; otherwise returns the right-hand operand.\n- `?.` (null conditional) — accesses a member of an object only if the object is not null, returning null instead of throwing a `NullReferenceException`.',
        notes:
          'There is also the null coalescing assignment operator `??=`, which assigns a value only if the variable is null.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'It means bundling data together with the methods that operate on that data, and restricting access to internal state.',
      ...
      },
      {
        ...
        notes: 'A LINQ provider is any class that implements `IQueryProvider` and `IQueryable`.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes: 'Member methods have priority over extension methods.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'If we want to prevent the modification of a collection, we can return it as `IEnumerable` or any other read-only collection type.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'The default for reference types is always comparing by reference. `==` is not supported by value types by default, but it can be overloaded.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- **Shallow copy** — value type members are copied, but for reference type members only the reference is copied.\n- **Deep copy** — reference type members are also copied into new objects.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'Memory leaks happen when memory is not freed even if an object is no longer used. The GC does not guarantee protection from them.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          "A nullable type is any type that can be assigned `null`. The `Nullable<T>` struct is a wrapper for value types that allows assigning null to a variable of that type. For example, you can't assign null to an `int`, but you can to a variable of type `Nullable<int>`.",
        notes:
          'The `Nullable<T>` struct has a constraint on `T` that requires it to be a value type.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'A property is a member that provides a mechanism for reading, writing, or computing the value of a private field.',
        notes:
          'Benefits of using a property:\n\n- Encapsulation.\n- Custom behavior on reading and writing.\n- Different access modifiers for reading and writing.\n- Properties can be calculated based on other data.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'Generic classes or methods are parametrized by type — for example, a `List<T>` can store elements of any type.',
        notes:
          'Type constraints allow limiting a generic type parameter only to types that meet specific criteria.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          "1. `const` fields are assigned at compile time; `readonly` fields can be assigned at runtime, in the constructor.\n2. `const` can only be numbers, booleans, strings, or a null reference; `readonly` values can be anything.\n3. `const` can't be declared `static`, because it is implicitly static; `readonly` fields can be static.",
        notes:
          'Use `const` when the value is known at compile time, and `readonly` when it is only known at runtime.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- `ref` passes a variable to a method by reference, so any modifications inside the method are visible outside it.\n- `out` is a way of returning extra values from a method.',
        notes:
          'You can use `out` to return more than one value from a method — consider the `TryParse` pattern as an example.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- **Interface** — defines the set of operations any implementing class must provide; it provides no implementation of its own.\n- **Abstract class** — a general blueprint for derived classes; it may provide method implementations, contain fields, etc.',
        notes:
          'An abstract class with a private constructor makes no sense, since abstract classes are meant to be inherited.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          'One thing, many forms: one interface, many implementations. The same method name behaves differently depending on the object.',
        notes:
          "Two types of polymorphism:\n\n- **Overloading** — at compile time; same name, different parameters.\n- **Overriding** — at runtime; redefines the parent's method.",
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- A `virtual` method **may** be overridden in a derived class.\n- An `abstract` method **must** be overridden (unless the derived class is abstract itself).',
        notes:
          'All abstract methods are implicitly virtual. You can use the `sealed` keyword on the overriding method to prevent further overriding.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'We can overload methods with optional parameters, but the method with no optional parameters will be used because its signature is closest to what was called.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '- **Overriding** — the derived class provides its own implementation of a `virtual` or `abstract` method from the base class.\n- **Hiding** — the derived class declares a method with the same name as a base class method, without overriding it.',
        notes:
          "Method hiding can help provide our own implementation of an external library's method. Note: the hidden method is used only when called through the derived type; otherwise the virtual method is used.",
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'The **diamond problem** is an ambiguity that arises when class `D` is derived from classes `B` and `C`, which both derive from class `A`. When both `B` and `C` override a method from `A`, it is ambiguous which implementation is used when the method is called on a `D` object.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          "A piece of business knowledge shouldn't be duplicated — it is not necessarily code duplication, but duplicated rules.",
        tags: [],
        nextSession: 0,
      },
      {
      ...
      },
      {
        ...
        answer:
          'The `goto` statement transfers program execution directly to a labeled statement. Using it is widely considered bad practice because it increases the complexity of the code: the flow becomes tricky to follow, making reading and debugging harder, and it can lead to unintentional infinite loops.',
        notes:
          'It can be useful to break out of nested loops when cleaning up logic or reviewing performance optimizations — not as a final implementation.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          '**Lasagna code** is code whose layers are complicated and so are the interactions between them — making a change in one layer heavily affects other layers. Long code is not an issue; tangled, messy, complicated code is.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'Singleton is considered an antipattern because it is a piece of global state, and global state is hard to control. Global state is a hidden dependency.',
        tags: [],
        nextSession: 0,
      },
      {
      ...
      },
      {
        ...
        answer:
          'Adapter is a design pattern that allows converting the interface of a class to the interface expected by a client.',
        tags: [],
        nextSession: 0,
      },
      {
      ...
      },
      {
        ...
        notes:
          'A static factory method makes code more readable, and is different from the Factory Method design pattern.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '**Single Responsibility Principle** — a class should be responsible for only one thing. An alternative definition: a class should have no more than one reason to change.',
        notes:
          'SRP gives us:\n\n- Smaller, more cohesive, more readable classes.\n- Reusable code.\n- Easier maintenance.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          '**Open-Closed Principle** — modules, classes, and functions should be open for extension, but closed for modification.',
      ...
      },
      {
        ...
        answer:
          '**Liskov Substitution Principle** — we should be able to use a derived type in place of its base type without knowing it, and it should not lead to any unexpected results.',
        tags: [],
        nextSession: 0,
      },
      {
        ...
        answer:
          "**Interface Segregation Principle** — clients of an interface should not be forced to depend on methods they don't use.",
        tags: [],
        nextSession: 0,
      },
      {
        ...
        notes:
          'The LSP tells us how to implement the interface of a base type; the ISP tells us whether we should implement that interface at all.',
        tags: [],
        nextSession: 0,
      },
      {
        question: 'When would you use a struct?',
        answer:
          '- When objects of this type will commonly be short-lived.\n- When the objects will be immutable.',
        notes:
  ...
  },
  {
    ...
    cards: [
      {
        question: 'What is the difference between `Tuple` and `ValueTuple`?',
        answer:
          '- `Tuple` (`System.Tuple`) is a **reference type** with immutable, unnamed properties (`Item1`, `Item2`, etc.).\n- `ValueTuple` (`System.ValueTuple`) is a **mutable value type** that supports named elements via language syntax.',
        notes:
      ...
      {
        question: 'What is the difference between the `is` and `as` keywords?',
        answer:
          '- `is` checks whether an object is compatible with a given type and returns a `bool` (optionally combined with a pattern-matching cast).\n- `as` attempts a cast and returns null if it fails, instead of throwing.',
        notes:
      ...
      },
      {
        ...
        answer:
          'The `using` keyword has two uses:\n\n1. A **using directive** that imports a namespace.\n2. A **using statement/declaration** that ensures an `IDisposable` object is disposed once it goes out of scope.',
        notes:
      ...
      },
      {
        ...
        answer:
          '- `Func` is a built-in generic delegate type representing a function with a given signature that returns a value.\n- A **lambda expression** is a concise, inline way to define an anonymous function, often assigned to a `Func` or `Action`.',
      ...
      },
      {
        ...
        answer:
          'The GC organizes the managed heap into three generations based on object age:\n\n- **Generation 0** — where short-lived objects start; collected most often.\n- **Generations 1 and 2** — long-lived objects are promoted here; collected less frequently.',
        notes:
      ...
      },
      {
        ...
        answer:
          "- `Dispose` is called explicitly (often via `using`) to deterministically release unmanaged resources.\n- `Finalize` is invoked by the GC nondeterministically, as a safety net before an object's memory is reclaimed.",
        notes:
      ...
      {
        question: 'Why is `catch (Exception)` almost always a bad idea (and when is it not)?',
        answer:
          "Catching the base `Exception` type hides the specific error and makes it easy to accidentally swallow bugs you didn't anticipate. It is generally acceptable only:\n\n- In a top-level global handler.\n- When immediately rethrowing.",
      ...
      },
      {
        ...
        answer:
          '- `throw` rethrows the current exception, preserving its original stack trace.\n- `throw ex` resets the stack trace to the current `catch` block, hiding where the exception actually originated.',
      ...
      },
      {
        ...
        answer:
          '- `typeof` is a **compile-time** operator that takes a type name and returns its `Type` object.\n- `GetType` is an **instance method** (inherited from `object`) that returns the runtime type of an object.',
        notes:
      ...
      },
      {
        ...
        answer:
          '- `double` is a 64-bit binary floating-point type with a wide range but rounding imprecision.\n- `decimal` is a 128-bit type optimized for base-10 arithmetic, giving exact precision for financial values at the cost of range and speed.',
      ...
      },
      {
        ...
        answer:
          '`ArrayList` is a non-generic, dynamically-resizable collection that stores elements as `object`, allowing mixed types but requiring boxing/unboxing and casting.',
        notes:
      ...
      },
      {
        ...
        notes:
          'Even when two keys share a hash code, `Equals` is used to correctly tell them apart.',
      ...
      },
      {
        ...
        answer:
          'Records (C# 9) and record structs (C# 10) are reference and value types, respectively, that come with:\n\n- Built-in value-based equality.\n- A generated `ToString`.\n- Support for non-destructive mutation via the `with` keyword.',
        notes:
      ...
      },
      {
        ...
        answer:
          '- `string` is immutable, so repeated concatenation creates a new object each time.\n- `StringBuilder` is mutable and designed for efficiently building up large strings incrementally, such as inside a loop.',
        notes:
      ...
      },
      {
        ...
        notes:
          'Explicit conversions require a cast in code; implicit conversions happen automatically.',
      ...
      },
      {
        ...
        answer:
          '- **Debug** builds skip compiler optimizations and keep full symbol information, making step-through debugging accurate.\n- **Release** builds apply optimizations for smaller, faster output at the cost of debuggability.',
        notes:
