...
export const DEFAULT_DECKS: DefaultDeckSeed[] = [
  ...
  },
  {
    deck: {
      ...
      description: 'Intermediate C# and .NET interview topics: generics, delegates, GC, reflection, patterns, and more.',
      tags: ['csharp', 'dotnet', 'interview'],
      sessionCount: 0,
    },
    cards: [
      {
        ...
        answer: 'Tuple (System.Tuple) is a reference type with immutable, unnamed properties (Item1, Item2, etc.), while ValueTuple (System.ValueTuple) is a mutable value type that supports named elements via language syntax.',
        notes: 'ValueTuples have been generally preferred since C# 7 for lightweight, temporary data grouping.',
        tags: ['csharp', 'tuples', 'value-types'],
        nextSession: 0,
      },
      {
        question: 'What is the difference between "is" and "as" keywords?',
        answer: '"is" checks whether an object is compatible with a given type and returns a bool (optionally combined with a pattern-matching cast); "as" attempts a cast and returns null if it fails instead of throwing.',
        notes: '"as" only works with reference types and nullable value types; a regular cast throws InvalidCastException on failure.',
        tags: ['csharp', 'casting', 'type-checking'],
        nextSession: 0,
      },
      {
        question: 'What is the use of the "using" keyword?',
        answer: 'The "using" keyword has two uses: a using directive that imports a namespace, and a using statement/declaration that ensures an IDisposable object is disposed once it goes out of scope.',
        notes: 'C# 8 added "using" declarations that dispose at the end of the enclosing scope, reducing nesting versus the older using(){} block.',
        tags: ['csharp', 'resource-management', 'idisposable'],
        nextSession: 0,
      },
      {
        question: 'What is the purpose of the "dynamic" keyword?',
        answer: 'The "dynamic" keyword tells the compiler to skip static type checking for that variable; type resolution happens at runtime instead.',
        notes: 'Useful for interop scenarios like COM objects, but it sacrifices compile-time safety and has a performance cost.',
        tags: ['csharp', 'dynamic-typing', 'interop'],
        nextSession: 0,
      },
      {
        question: 'What are expression-bodied members?',
        answer: 'Expression-bodied members let you define a method, property, or other member using a single expression with the "=>" syntax instead of a full method body.',
        notes: 'Handy for reducing boilerplate on simple one-line members such as getters or short calculations.',
        tags: ['csharp', 'syntax', 'members'],
        nextSession: 0,
      },
      {
        question: 'What are Funcs and lambda expressions?',
        answer: 'Func is a built-in generic delegate type representing a function with a given signature that returns a value; a lambda expression is a concise, inline way to define an anonymous function, often assigned to a Func or Action.',
      ...
      },
      {
        ...
        answer: 'A delegate is a type-safe reference to one or more methods with a matching signature, letting methods be passed as parameters, stored in variables, or invoked indirectly.',
        notes: 'Multicast delegates hold references to more than one method and invoke them all in sequence when called.',
      ...
      },
      {
        ...
        answer: "The Garbage Collector treats an object as removable once no reachable reference chain exists from the application's roots to that object; it traces reachability rather than counting references.",
        notes: ".NET uses a tracing (mark-and-sweep) GC, unlike reference-counting systems such as Swift's ARC.",
        tags: ['csharp', 'garbage-collection', 'memory'],
        nextSession: 0,
      },
      {
        question: 'What are generations?',
        answer: 'The GC organizes the managed heap into three generations (0, 1, 2) based on object age; short-lived objects start in generation 0 and are collected most often, while long-lived objects get promoted to higher generations that are collected less frequently.',
        notes: 'The Large Object Heap holds objects over roughly 85,000 bytes and is logically treated as part of generation 2.',
        tags: ['csharp', 'garbage-collection', 'memory'],
        nextSession: 0,
      },
      {
        question: 'What is the difference between Dispose and Finalize methods?',
        answer: "Dispose is called explicitly (often via \"using\") to deterministically release unmanaged resources, while Finalize is invoked by the GC nondeterministically as a safety net before an object's memory is reclaimed.",
        notes: 'Implementing IDisposable and calling GC.SuppressFinalize avoids the extra overhead of an unnecessary finalizer call.',
        tags: ['csharp', 'idisposable', 'garbage-collection'],
        nextSession: 0,
      },
      {
        question: 'What are default implementations in interfaces?',
        answer: 'Since C# 8, interfaces can provide a default method body, letting new members be added to an interface without breaking classes that already implement it.',
        notes: 'Mainly intended for evolving public library interfaces over time, not as a general substitute for abstract classes.',
        tags: ['csharp', 'interfaces', 'csharp8'],
        nextSession: 0,
      },
      {
        question: 'What is deconstruction?',
        answer: 'Deconstruction lets you split an object such as a tuple, ValueTuple, or record into individual variables in a single statement using a Deconstruct method.',
        notes: "You can add a Deconstruct method to your own types, or supply one as an extension method for types you don't own.",
        tags: ['csharp', 'tuples', 'records'],
        nextSession: 0,
      },
      {
        question: 'Why is "catch(Exception)" almost always a bad idea (and when it is not?)?',
        answer: "Catching the base Exception type hides the specific error type and makes it easy to accidentally swallow bugs you didn't anticipate; it's generally acceptable only in a top-level global handler or when immediately rethrowing.",
      ...
      },
      {
        ...
        answer: '"throw" rethrows the current exception while preserving its original stack trace; "throw ex" resets the stack trace to the current catch block, hiding where the exception actually originated.',
      ...
      },
      {
        ...
        answer: 'typeof is a compile-time operator that takes a type name and returns its Type object; GetType is an instance method (inherited from object) that returns the runtime type of an object.',
        notes: 'typeof needs the type name known at compile time, while GetType reflects the actual runtime type of a polymorphic instance.',
        tags: ['csharp', 'reflection', 'types'],
        nextSession: 0,
      },
      {
        question: 'What is reflection?',
        answer: 'Reflection is a mechanism that lets code inspect and interact with type metadata at runtime, such as listing members, reading attributes, or invoking a method by name.',
        notes: 'Powerful but has a real performance cost and can make code harder to understand and maintain.',
        tags: ['csharp', 'reflection', 'metadata'],
        nextSession: 0,
      },
      {
        question: 'What are attributes?',
        answer: 'Attributes are declarative metadata tags attached to types or members that can be read at runtime via reflection, such as the built-in [Serializable] or custom attributes deriving from Attribute.',
        notes: 'Commonly used for serialization hints, validation rules, or framework/tooling configuration.',
        tags: ['csharp', 'attributes', 'metadata'],
        nextSession: 0,
      },
      {
        question: 'What is serialization?',
        answer: "Serialization is the process of converting an object's state into a storable or transmittable format such as JSON, XML, or binary; deserialization reverses the process.",
        notes: "Common uses include sending data over a network, persisting it to disk, or caching a snapshot of an object's state.",
        tags: ['csharp', 'serialization', 'data'],
        nextSession: 0,
      },
      {
        question: 'What is pattern matching?',
        answer: 'Pattern matching lets you test whether an expression has a particular shape or type and, if so, extract data from it in the same statement, e.g. "if (obj is string text)".',
        notes: 'C# has significantly expanded pattern matching (switch expressions, property patterns, and more) since C# 7.',
        tags: ['csharp', 'pattern-matching', 'syntax'],
        nextSession: 0,
      },
      {
        question: 'How does the binary number system work?',
        answer: 'The binary number system represents values using only 0s and 1s, where each bit position corresponds to a power of two; computers use it natively because a bit maps directly onto two physical states.',
        notes: "Arithmetic overflow happens when a result needs more bits than a numeric type's fixed size allows.",
        tags: ['csharp', 'fundamentals', 'numbers'],
        nextSession: 0,
      },
      {
        question: 'What is the purpose of the "checked" keyword?',
        answer: 'The "checked" keyword marks a block or expression so that arithmetic overflow throws an OverflowException instead of silently wrapping around.',
        notes: 'The default in most build configurations is unchecked; use checked for money or other safety-critical calculations.',
        tags: ['csharp', 'arithmetic', 'overflow'],
        nextSession: 0,
      },
      {
        question: 'What is the difference between double and decimal?',
        answer: 'double is a 64-bit binary floating-point type with a wide range but rounding imprecision; decimal is a 128-bit type optimized for base-10 arithmetic, giving exact precision for financial values at the cost of range and speed.',
      ...
      },
      {
        ...
        answer: "An Array is C#'s most basic collection type, storing a fixed number of elements of the same type in contiguous memory.",
        notes: "Arrays can't be resized after creation; use List<T> when the collection needs to grow or shrink.",
        tags: ['csharp', 'collections', 'arrays'],
        nextSession: 0,
      },
      {
        question: 'What is a List?',
        answer: 'List<T> is a generic, strongly-typed, dynamically-resizable collection built on top of an array that automatically grows its backing array as elements are added.',
        notes: 'Setting an expected Capacity upfront avoids the costly array-copy operations that happen as the list grows.',
        tags: ['csharp', 'collections', 'generics'],
        nextSession: 0,
      },
      {
        question: 'What is an ArrayList?',
        answer: 'ArrayList is a non-generic, dynamically-resizable collection that stores elements as System.Object, allowing mixed types but requiring boxing/unboxing and casting.',
        notes: 'Considered obsolete in modern C#; List<T> should be used instead in virtually all cases.',
        tags: ['csharp', 'collections', 'legacy'],
        nextSession: 0,
      },
      {
        question: 'What is the purpose of the GetHashCode method?',
        answer: 'GetHashCode returns an integer "fingerprint" for an object that hash-based collections (like Dictionary and HashSet) use to bucket and quickly locate items.',
        notes: 'If you override Equals, you must also override GetHashCode consistently, since equal objects must produce equal hash codes.',
        tags: ['csharp', 'hashing', 'collections'],
        nextSession: 0,
      },
      {
        question: 'What is a Dictionary?',
        answer: 'Dictionary<TKey, TValue> is a hash-table-based collection that stores key-value pairs and provides near O(1) average-case lookup, insertion, and deletion by key.',
      ...
      },
      {
        ...
        answer: 'Indexers let instances of a class be accessed with array-like bracket syntax (e.g. obj[key]) by defining a special "this[...]" property.',
        notes: 'A class can define multiple overloaded indexers as long as their parameter lists differ, similar to method overloading.',
        tags: ['csharp', 'syntax', 'indexers'],
        nextSession: 0,
      },
      {
        question: 'What is caching?',
        answer: 'Caching is the practice of storing the results of expensive operations (data fetches or calculations) in a fast-access store keyed by some identifier, so repeated requests can be served without redoing the work.',
        notes: 'Caches need an eviction or expiration strategy to avoid unbounded memory growth and serving stale data.',
        tags: ['csharp', 'performance', 'caching'],
        nextSession: 0,
      },
      {
        question: "What are immutable types and what's their purpose?",
        answer: "An immutable type's state can't change after construction; any \"modification\" instead produces a new object, which simplifies reasoning, testing, and multithreaded code.",
        notes: "A \"non-destructive mutation\" (e.g. a record's \"with\" expression) creates a new object rather than altering the original.",
        tags: ['csharp', 'immutability', 'design'],
        nextSession: 0,
      },
      {
        question: 'What are records and record structs?',
        answer: 'Records (C# 9) and record structs (C# 10) are reference and value types, respectively, that come with built-in value-based equality, a generated ToString, and support for non-destructive mutation via the "with" keyword.',
        notes: '"Positional records" auto-generate a constructor, properties, and a Deconstruct method from a concise one-line declaration.',
        tags: ['csharp', 'records', 'csharp9'],
        nextSession: 0,
      },
      {
        question: 'Why does string behave like a value type even though it is a reference type?',
        answer: "string is technically a reference type, but because it's immutable, any operation that appears to modify it actually creates a new string object, giving it value-type-like behavior in practice.",
        notes: 'String interning lets equal string literals share the same underlying object to save memory, which is only safe because strings are immutable.',
        tags: ['csharp', 'strings', 'reference-types'],
        nextSession: 0,
      },
      {
        question: 'What is the difference between string and StringBuilder?',
        answer: 'string is immutable, so repeated concatenation creates a new object each time; StringBuilder is mutable and designed for efficiently building up large strings incrementally, such as inside a loop.',
        notes: "For a single concatenation, a plain string is fine; StringBuilder pays off once you're doing many repeated appends.",
        tags: ['csharp', 'strings', 'performance'],
        nextSession: 0,
      },
      {
        question: 'What is operator overloading?',
        answer: 'Operator overloading lets a type define custom behavior for operators (+, -, ==, etc.) using the "operator" keyword, including implicit and explicit conversion operators.',
        notes: 'Explicit conversions require an explicit cast in code; implicit conversions happen automatically.',
        tags: ['csharp', 'operators', 'syntax'],
        nextSession: 0,
      },
      {
        question: 'What are anonymous types?',
        answer: 'Anonymous types are compiler-generated, unnamed reference types created with "new { ... }" that hold a fixed set of read-only properties, commonly used for temporary projections in LINQ queries.',
        notes: 'Two anonymous objects with identical property values and order are considered equal via Equals despite being different instances.',
        tags: ['csharp', 'linq', 'types'],
        nextSession: 0,
      },
      {
        question: 'What is cohesion?',
        answer: 'Cohesion describes how closely related and focused the responsibilities of a single module or class are; high cohesion means related data and behavior are grouped together.',
        notes: "High cohesion often correlates with, but isn't identical to, following the Single Responsibility Principle.",
        tags: ['csharp', 'design-principles', 'architecture'],
        nextSession: 0,
      },
      {
        question: 'What is coupling?',
        answer: 'Coupling describes how much one module or class depends on the internal details of another; low (loose) coupling makes a codebase easier to change and test.',
        notes: 'The Dependency Inversion Principle — depend on abstractions, not concretions — is a key technique for reducing coupling.',
        tags: ['csharp', 'design-principles', 'solid'],
        nextSession: 0,
      },
      {
        question: 'What is the Strategy design pattern?',
        answer: 'The Strategy pattern defines a family of interchangeable algorithms behind a common interface, letting the concrete algorithm be selected or swapped at runtime.',
        notes: 'It decouples the code that varies (the algorithm) from the code that stays the same (the code that uses it).',
        tags: ['csharp', 'design-patterns', 'strategy'],
        nextSession: 0,
      },
      {
        question: 'What is the Dependency Injection design pattern?',
        answer: 'Dependency Injection is a technique where a class receives (is "injected with") the objects it depends on from the outside, typically via its constructor, rather than creating them itself.',
      ...
      },
      {
        ...
        answer: 'The Template Method pattern defines the overall skeleton of an algorithm in a base class while letting subclasses override specific steps of that algorithm.',
        notes: 'Unlike Strategy, which uses composition and runtime selection, Template Method uses inheritance and is fixed at compile time.',
        tags: ['csharp', 'design-patterns', 'template-method'],
        nextSession: 0,
      },
      {
        question: 'What is the Decorator design pattern?',
        answer: 'The Decorator pattern lets you add new behavior to an object dynamically by wrapping it in one or more decorator objects that implement the same interface.',
        notes: "It supports the Open-Closed Principle, since existing classes don't need to be modified to add functionality.",
        tags: ['csharp', 'design-patterns', 'decorator'],
        nextSession: 0,
      },
      {
        question: 'What is the Observer design pattern?',
        answer: 'The Observer pattern lets one object (the Observable or Subject) automatically notify a set of subscribed objects (Observers) whenever its state changes.',
      ...
      },
      {
        ...
        answer: "Events are .NET's language-level implementation of the Observer pattern, built on delegates, letting a class publish notifications that other code can subscribe to and react on.",
        notes: 'Unlike a public delegate field, an event can only be raised from within the declaring class; forgetting to unsubscribe can also leak memory by keeping the subscriber alive.',
        tags: ['csharp', 'events', 'delegates'],
        nextSession: 0,
      },
      {
        question: 'What is Inversion of Control?',
        answer: 'Inversion of Control is a design principle where the flow of control is handed over to a framework or container, which calls into your code at defined extension points, rather than your code driving everything itself.',
        notes: "Dependency Injection is one common technique for implementing IoC, and it's what distinguishes a framework from a plain library.",
        tags: ['csharp', 'inversion-of-control', 'architecture'],
        nextSession: 0,
      },
      {
        question: 'What is the "composition over inheritance" principle?',
        answer: '"Composition over inheritance" favors building types by combining smaller objects (composition) rather than through class hierarchies, since composition tends to be more flexible and avoids fragile base-class problems.',
        notes: 'Relying purely on composition can make genuine "is-a" relationships awkward, sometimes leading to lots of thin forwarding methods.',
        tags: ['csharp', 'design-principles', 'composition'],
        nextSession: 0,
      },
      {
        question: 'What are mocks?',
        answer: 'Mocks are fake, controllable stand-ins for real dependencies used in unit tests, letting you verify interactions and control returned values without invoking real implementations.',
        notes: 'Mocking generally requires Dependency Injection so real dependencies can be swapped for mocks in tests.',
        tags: ['csharp', 'testing', 'mocking'],
        nextSession: 0,
      },
      {
        question: 'What are NuGet packages?',
        answer: 'NuGet packages are the standard way to distribute and consume reusable, precompiled .NET code and libraries, installed and restored via the NuGet package manager or "dotnet restore".',
        notes: "Before writing something generic yourself, it's usually worth checking whether a NuGet package already solves it.",
        tags: ['csharp', 'nuget', 'ecosystem'],
        nextSession: 0,
      },
      {
        question: 'What is the difference between Debug and Release builds?',
        answer: 'Debug builds skip compiler optimizations and keep full symbol information, making step-through debugging accurate; Release builds apply optimizations for smaller, faster output at the cost of debuggability.',
        notes: 'You can gate code to run only in one configuration using #if DEBUG / #if RELEASE preprocessor directives.',
        tags: ['csharp', 'build-configuration', 'debugging'],
        nextSession: 0,
      },
      {
        question: 'What are preprocessor directives?',
        answer: 'Preprocessor directives (like #if, #define, #region, #pragma) are instructions processed before compilation that can conditionally include code, define symbols, or suppress specific compiler warnings.',
        notes: 'Example: #pragma warning disable CA2200 turns off a specific compiler warning by its code.',
        tags: ['csharp', 'preprocessor', 'compiler'],
        nextSession: 0,
      },
      {
        question: 'What are nullable reference types?',
        answer: 'Nullable reference types (C# 8) let the compiler track and warn about reference variables that might be null, extending to reference types the kind of nullability annotations value types already had with Nullable<T>.',
        notes: "It's a compile-time analysis feature based on warnings, not a runtime enforcement mechanism — null checks can still be bypassed.",
